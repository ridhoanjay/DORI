...
└───────────────⭓`;
    ...
    const cap =
      Func.Styles(`Hai`) +
      ` @${m.sender.split("@")[0]} ` +
      Func.Styles(`Perkenalkan nama saya Dori, saya adalah asisten pintar yang siap membantu Anda dengan fitur-fitur yang saya punya ^^
...
└───────────────⭓\n*[ ${data.premium ? "✓" : "x"} ]* Premium Only\n\n${data.example ? `「 *Penggunaan* 」\n${data.example.replace("%cmd", usedPrefix + text)}` : ""}\n\n${data.description ? data.description : "Tidak ada deskripsi"}`);
  ...
  error: 0,
  update: Date.now(),
  description: "Menampilkan semua fitur Dori",
};
